	...
	#Hier jetzt alle Return Codes ueberpruefen
			...
			(else_try),
				...
				# Set position
				 ...
				 (try_begin),
				 	(gt, ":horse", 340),
				 	(player_set_slot, ":player_id", slot_player_horse_skin, ":horse"),
				 (try_end),
		...
		(try_begin), # failures in this section will force disconnection from the scene prop, to stop receiving updates
			...
			(try_begin), # for failures after this point, stay connected with the scene prop, to receive updates
				...
				(else_try),
					...
					(prop_instance_get_variation_id_2, reg100, ":instance_id"),
					(server_add_message_to_log, "@{s99} took {s98} from chest {reg100}"),
				(else_try),
					...
					(str_store_player_username, s99, ":player_id"),
					(prop_instance_get_variation_id_2, reg100, ":instance_id"),
					(server_add_message_to_log, "@{s99} deposited {s98} to chest {reg100}"),
				(try_end),
...
("wse_chat_message_received", [
	...
	(try_begin),
		...
		(player_slot_eq, ":player_no", slot_player_admin_no_all_items, 0),
		(str_split, reg109, s0, s0, "@ "),
		(try_begin),
			(str_starts_with, s0, "@/stock"),
			(str_to_num, reg108, s1),
			(try_for_prop_instances, ":cur_prop"),
				(scene_prop_get_slot, ":current_stock", ":cur_prop", slot_scene_prop_stock_count),
				(val_add, ":current_stock", reg108),
				(scene_prop_set_slot, ":cur_prop", slot_scene_prop_stock_count, ":current_stock"),
			(try_end),
		(else_try),
			...
			(agent_is_active, ":agent_id"),
			(agent_is_alive, ":agent_id"),
			(agent_set_speed_modifier, ":agent_id", reg108),
		(try_end),
	(try_end),
]),
...
("is_skin", [
	(store_script_param, ":item_id", 1),
	(try_begin),
		(this_or_next|eq, ":item_id", 1201),
		(this_or_next|eq, ":item_id", 1202),
		(is_between, ":item_id", "itm_strange_armor_skin", "itm_cw_plate_falcon_hood"),
		(neq, ":item_id", "itm_light_charger"),
		(assign, reg97, 1),
	(else_try),
		(assign, reg97, 0),
	(try_end),
