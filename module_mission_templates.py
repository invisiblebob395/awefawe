...
##Arthur: no more spectator if not admin
no_spec_cam = (0, 0, 1, [(neg|multiplayer_is_server),],
...
before_mission_start_setup = (ti_before_mission_start, 0, 0, [], # set up basic mission and scene values
   [(server_set_friendly_fire, 1),
    ...
    (team_set_relation, team_spawn_invulnerable, team_bots, -1),
    (team_set_relation, team_bots, team_bots, 1),
    (team_set_relation, team_bots, team_default, -1),
    (team_set_relation, team_bots, team_spawn_invulnerable, -1),
...
after_mission_start_setup = (ti_after_mission_start, 0, 0, [], # spawn and move certain things after most other set up is done
   [(set_spawn_effector_scene_prop_kind, 0, -1),
    ...
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg20}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg21}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg22}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg23}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg24}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg25}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg26}"),
    (send_message_to_url, "@http://localhost/backendDsDvC/loadfaction.php?faction={reg27}"),

    (call_script, "script_cf_load_all_chests"), #GGG:house chest
    (call_script, "script_after_mission_start_spawn_bots"),#GGG:bots
    ])

player_joined = (ti_server_player_joined, 0, 0, [], # server: handle connecting players
   [(store_trigger_param_1, ":player_id"),
    (player_is_active, ":player_id"),
    #(multiplayer_send_3_int_to_player, ":player_id", server_event_player_set_cam, 0, 0, 0),
    ...
    (try_begin),
      ...
      (try_for_range, ":cur_player", 0, ":max_players"),
        ...
        (player_get_slot, ":outlaw_rating", ":cur_player", slot_player_outlaw_rating),
        (multiplayer_send_3_int_to_player, ":player_id", server_event_player_set_slot, ":cur_player", slot_player_outlaw_rating, ":outlaw_rating"),

        (player_get_slot, ":title_number", ":cur_player", slot_player_title),
        (multiplayer_send_3_int_to_player, ":player_id", server_event_player_set_slot, ":cur_player", slot_player_title, ":title_number"), #GGG:title
      (try_end),
    (try_end),
    ])
...
agent_spawn = (ti_on_agent_spawn, 0, 0, [], # server and clients: set up new agents after they spawn
   [(store_trigger_param_1, ":agent_id"),
    ...
    (try_begin),
      ...
      (this_or_next|eq, ":troop_id", "trp_royal_archer"),
      (this_or_next|eq, ":troop_id", "trp_royal_man_at_arms"),
      (this_or_next|eq, ":troop_id", "trp_royal_footman"),
      (eq, ":troop_id", "trp_royal_lord"),
    ...
    (player_is_active, ":player_id"),     
  	(try_begin),
  	  (neg|player_is_admin, ":player_id"),
      (call_script, "script_player_get_armor", ":player_id"), #gets the player armor
  	(try_end),
...
agent_killed = (ti_on_agent_killed_or_wounded, 0, 0, [], # server and clients: handle messages, score, loot, and more after agents die
   [(store_trigger_param_1, ":dead_agent_id"),
    ...
    #Arthur: prevent click through by setting team
    #(try_begin),
    ...
    #(try_end),
    #(try_begin),
    #  (neg|agent_is_alive, ":dead_agent_id"),
    #  (agent_get_player_id, ":player_id", ":dead_agent_id"),
    #  (agent_get_position, pos0, ":dead_agent_id"),
    #  (position_get_x, ":x", pos0),
    #  (position_get_y, ":y", pos0),
    #  (position_get_z, ":z", pos0),
      #(multiplayer_send_3_int_to_player, ":player_id", server_event_player_set_cam, ":x", ":y", ":z"),
    #(try_end),
    (try_begin),
    #GGG:bots
      (agent_is_active, ":dead_agent_id"),
      (agent_is_active, ":killer_agent_id"),
      (agent_get_player_id, ":player_id", ":killer_agent_id"),
      (gt, ":player_id", -1),
      (agent_is_non_player, ":dead_agent_id"),
      (agent_is_human, ":dead_agent_id"),
      (agent_get_troop_id, ":troop_id", ":dead_agent_id"),
      (try_begin),
        (eq, ":troop_id", "trp_royal_archer"),
        (assign, ":reward", 5000),
      (else_try),
        (eq, ":troop_id", "trp_royal_footman"),
        (assign, ":reward", 6000),
      (else_try),
        (eq, ":troop_id", "trp_royal_man_at_arms"),
        (assign, ":reward", 7000),
      (else_try),
        (eq, ":troop_id", "trp_royal_lord"),
        (assign, ":reward", 10000),
      (try_end),
      (call_script, "script_player_adjust_gold", ":player_id", ":reward", 1),
      (call_script, "script_check_spawn_bots", ":dead_agent_id"),
    #
    (else_try),
    #GGG:outlaw rating
      (agent_get_troop_id, ":troop_id", ":dead_agent_id"),
      (eq, ":troop_id", "trp_temp_troop"),
      (agent_get_player_id, ":player_id", ":killer_agent_id"),
      (gt, ":player_id", -1),
    ...
    (else_try),
      (agent_get_player_id, ":player_id", ":dead_agent_id"),
      (gt, ":player_id", -1),
      #(player_get_slot, ":disconnected_bool", ":player_id", slot_player_has_disconnected_ok),
      (try_begin),
	      (neg|player_slot_eq, ":player_id", slot_player_check_passed, 1),
  		  ...
  		  (else_try),
    		  (agent_get_player_id, ":player_id", ":dead_agent_id"),
    		  (gt, ":player_id", -1), #muss mensch sein ansonsten nein

          (player_get_troop_id, ":troop", ":player_id"),
          (player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
          #Arthur reset troop if commoner and advanced troop if outlaw
          (try_begin),
            (multiplayer_is_server),
            (eq, ":faction_id", "fac_commoners"),
            (this_or_next|eq, ":troop", "trp_sergeant"),
            (this_or_next|eq, ":troop", "trp_brigand"),
            (this_or_next|eq, ":troop", "trp_man_at_arms"),
            (this_or_next|eq, ":troop", "trp_lancer"),
            (this_or_next|eq, ":troop", "trp_archer"),
            (eq, ":troop", "trp_crossbowman"),

            (server_add_message_to_log, "@You are a commoner"),
            (player_set_troop_id, ":player_id", "trp_peasant"),
          (else_try),
            (multiplayer_is_server),
            (eq, ":faction_id", "fac_outlaws"),
            (this_or_next|eq, ":troop", "trp_man_at_arms"),
            (eq, ":troop", "trp_sergeant"),
            (player_set_troop_id, ":player_id", "trp_brigand"),
          (try_end),

          
    		  (call_script, "script_player_death_update", ":player_id"), #removes gear from database of the player cause player died and didnt leave
    		...
    		#  (call_script, "script_check_spawn_bots", ":dead_agent_id"), not needed
          #GGG:outlaw rating
          (try_begin),
            (agent_is_active, ":killer_agent_id"), #GGG:debug
            (agent_get_troop_id, ":troop_id", ":killer_agent_id"),
            (eq, ":troop_id", "trp_huntsman"),
            ...
            (lt, ":outlaw_rating", 28),
            (store_mul, ":reward", ":outlaw_rating", 500),
            (call_script, "script_player_adjust_gold", ":killer_id", ":reward", 1),
            (call_script, "script_player_change_check_outlaw_rating", ":player_id", outlaw_rating_for_kill, 0),
          (try_end),
    ...
    (try_end),
	  ...
	  (call_script, "script_check_animal_killed", ":dead_agent_id", ":killer_agent_id"), #falls kein mensch gestorben ist dann war es pferd
    ])
   ...
   [(store_trigger_param_1, ":attacked_agent_id"),
    ...
    (else_try),
      ...
      (try_begin),
        ...
        (try_begin),
          (eq, ":body_armor", 43),
          (assign, ":reduced_damage", 15),
        (else_try),
          (eq, ":body_armor", 44),
          (assign, ":reduced_damage", 20),
        (else_try),
          (eq, ":body_armor", 45),
          (assign, ":reduced_damage", 25),
        (else_try),
          (eq, ":body_armor", 46),
          (assign, ":reduced_damage", 30),
        (else_try),
          (eq, ":body_armor", 47),
          (assign, ":reduced_damage", 35),
        (else_try),
          (eq, ":body_armor", 48),
          (assign, ":reduced_damage", 40),
        (else_try),
          (is_between, ":body_armor", 49, 55),
          (assign, ":reduced_damage", 45),
        (else_try),
          (ge, ":body_armor", 55),
          (assign, ":reduced_damage", 55),
        (else_try),
      ...
      #GGG:skl_riding
        ...
        (try_begin),
          (store_mul, ":reduced_damage", ":riding_level", 5),
          (val_mul, ":reduced_damage", ":damage_dealt"),
          (val_div, ":reduced_damage", 100),
          (val_sub, ":damage_dealt", ":reduced_damage"),
          (val_max, ":damage_dealt", 0),
   ...
   [(multiplayer_is_server),
    (try_begin), # if the loop was not restarted
      ...
      (try_for_agents, ":loop_agent_id"), # find the next agent id greater than the previous checked
        ...
        #  (val_min, ":food_amount", max_food_amount),
        #  (assign, ":modifier_number", ":food_amount"),
        #  (val_div, ":modifier_number", 10),
        #  (val_add, ":modifier_number", 100),
        #  (agent_set_damage_modifier, ":agent_id", ":modifier_number"),
        #(try_end), 
        #(try_begin),
        #  (agent_get_slot, ":water_amount", ":agent_id", slot_agent_water_amount),
      ...
      (try_begin),
        ...
        (try_begin),
          (eq, "$g_loop_hunger_check", 1),
          (call_script, "script_check_agent_hunger_and_thirst", ":agent_id"),
        (try_end),        
        ###
      (else_try),
    ...
    (else_try), # setting up to restart the loop
      ...
      (try_begin),
        ...
        ###GGG:hunger and thirst
        (try_begin),
          (ge, ":time", "$g_loop_hunger_check_time"),
          (val_add, "$g_loop_hunger_check_time", loop_hunger_check_interval),
          (assign, "$g_loop_hunger_check", 1),
        (else_try),
...
agent_check_attack_loop = (0, 0, 0.2, [], # server: repeatedly check all agents for attacking with a weapon they can't use - should be kept as simple as possible
   [(multiplayer_is_server),
    (try_for_agents, ":agent_id"),
      ...
      #  (agent_is_alive, ":agent_id"),
      #  (agent_is_human, ":agent_id"),
      #  (agent_get_position, pos57, ":agent_id"),
      #  (try_begin),
      ...
      ##    (agent_get_wielded_item, ":wielded_item_id", ":agent_id", 0),
       ...
       #   (neg|is_between, ":wielded_item_id", work_item_begin, work_item_end),
       #   (neq, ":wielded_item_id", "itm_wand_of_law"),
       #   (agent_get_attack_action, ":action", ":agent_id"),
       #   (is_between, ":action", 1, 7),
       #   (agent_set_wielded_item, ":agent_id", -1), 
       # (else_try),
       #   (get_distance_between_positions_in_meters, ":is_savearea", pos57, pos58),
       #   (le, ":is_savearea", 50),
       #   (agent_get_wielded_item, ":wielded_item_id", ":agent_id", 0),
       #   (gt, ":wielded_item_id", -1),
       #   (neg|is_between, ":wielded_item_id", work_item_begin, work_item_end),
...
polls_check = (2, 0, 0, [], # server: regularly check to see if any polls have ended
   [(multiplayer_is_server),
    ...
    #Combat state add/Combat log add:
    (try_begin),
  		(get_max_players, ":max_players"),
  		(val_add, ":max_players", 1),
  		
  		(try_for_range, ":current_player", 0, ":max_players"),
  			(player_is_active, ":current_player"),
        (try_begin),
    			(player_get_slot, ":state", ":current_player", slot_player_fight_state),
    			(ge, ":state", 1),
    			(val_sub, ":state", 1),
    			(player_set_slot, ":current_player", slot_player_fight_state, ":state"),
    			(multiplayer_send_3_int_to_player, ":current_player", server_event_player_set_slot, ":current_player", slot_player_exiting, ":state"),     #GGG:delay downline
    			#announcer
    			(eq, ":state", 0),
    			(str_store_string, s1, "@You can now safely log out!"),    
    			(multiplayer_send_string_to_player, ":current_player", server_event_local_chat, s1),

        #GGG:outlaw rating
        (else_try),
          (player_get_slot, ":not_protected", ":current_player", slot_player_fight_not_good),
          (ge, ":not_protected", 1),
          (val_sub, ":not_protected", 1),
          (player_set_slot, ":current_player", slot_player_fight_not_good, ":not_protected"),

          (eq, ":not_protected", 0),
          (str_store_string, s1, "@You is protected now."),    
          (multiplayer_send_string_to_player, ":current_player", server_event_local_chat, s1),
        (try_end),
        #
  		(try_end),
...
#GGG:house chest
auto_chest_save = (0, 60, 240, [(multiplayer_is_server),], #server: auto-saves all chests and factions each 5 minutes
   [(call_script, "script_cf_save_all_chests"),
    ...
    (try_begin),
      ...
      (try_for_range, ":hero_troop", "trp_hero_1", ":hero_end"),
        (assign, ":used", 0),
        (troop_slot_eq, ":hero_troop", slot_troop_hero_is_used, 1),
        (get_max_players, ":max_players"),
        ...
        (try_end),
        (eq, ":used", 0),
        (troop_set_slot, ":hero_troop", slot_troop_hero_is_used, 0),
      (try_end),
    (try_end),
      #
     ])
...
check_veriosn = (ti_on_agent_spawn, 0, 0, [],
   [
    #(store_trigger_param_1, ":agent_id"),
    #(agent_get_player_id, ":player_id", ":agent_id"),
    #(player_is_active, ":player_id"),
    #(ge, ":player_id", 0),
    #(neg|player_slot_eq, ":player_id", slot_player_have_access, 4555),

    #(str_store_string, s0, "@The verison is too old, please download the latest verison."),
...
#shield bash
shield_bash = (0, 0.35, 1,[(game_key_is_down, gk_defend), (key_clicked, key_middle_mouse_button),],
   [
...
#GGG:loot weapon
loot_weapon = (0, 0.55, 1,[(game_key_is_down, gk_attack),],
   [
    (game_key_is_down, gk_attack),
    (call_script, "script_loot_weapon_trigger")

    ])
###Arthur: cancel spec
#no_spectator = (0, 0, 1,[(neg|multiplayer_is_server), (this_or_next|game_key_is_down, gk_move_forward),(this_or_next|game_key_is_down, gk_move_backward), (this_or_next|game_key_is_down, gk_move_left), (this_or_next|game_key_is_down, gk_move_right), (this_or_next|game_key_is_down, gk_attack), (game_key_is_down, gk_defend),],
#   [
#    (multiplayer_get_my_player, ":player_id"),
#    (player_is_active, ":player_id"),
#    (player_get_agent_id, ":agent_id", ":player_id"),
...
#    (init_position, pos10),
#    (display_message, "@Trigger", 0xFFFF22),
#    (cur_tableau_set_camera_position, pos10),
...
AI_kick =  (2, 0, 0,[], 
   [
    ...
    (try_for_agents, ":agent"),
      ...
      (eq, ":horse", -1),
      (try_begin),
        (agent_get_ammo, ":agent_ammo", ":agent"),
        (le, ":agent_ammo", 1),
      ...
      (try_begin),
        ...
        (store_random_in_range,":kickchance", 1, 5),
        (try_begin),
          (eq,":kickchance", 1), 
...
death_blocker = (
  0.1, 0, 0, [(neg|multiplayer_is_server),],
  [

    (neg|is_presentation_active, "prsnt_death_blocker"),
    (multiplayer_get_my_player, ":player_id"),
    (neg|player_is_admin, ":player_id"),
    (player_get_agent_id, ":agent_id", ":player_id"),
    (assign, ":continue", 0),
    ...
    (try_end),
    (eq, ":continue", 1),
    (start_presentation, "prsnt_death_blocker"),
    ])
plus_server_block_ai_scr = (
  0.5, 0, 0,
  [
  (multiplayer_is_server),
  ],[
   ...
   [
    ...
    (ti_on_agent_spawn, 0, 0, [], # move new agents near the position of previous ones
     [(store_trigger_param_1, ":agent_id"),
      (neg|agent_is_non_player, ":agent_id"),
      #(agent_get_player_id, ":player_id", ":agent_id"),
      #(multiplayer_send_3_int_to_player, ":player_id", server_event_player_set_cam, 5123, 0, 0),
