...
#	]),
 ...
 ("cf_withdraw_bank",
	...
	(try_begin),
      ...
      (send_message_to_url, "@http://localhost/backendDsDvC/withdraw.php?guid={reg2}&playerid={reg0}&gold={reg1}&carried={reg8}"),
	...
	(try_begin),
	  ...
	  (player_get_gold, ":original_player_gold", ":player_id"),
	  (assign, reg8, ":original_player_gold"),
      (send_message_to_url, "@http://localhost/backendDsDvC/deposit.php?guid={reg2}&playerid={reg0}&gold={reg1}&carried={reg8}"),
      (str_store_player_username, s0, ":player_id"),
    (try_end),
	]),
...
##############################################
  ...
  ("game_receive_url_response", # called by the game when a response is received from a web server, if used
   [(store_script_param, ":integer_count", 1),
    #(store_script_param, ":string_count", 2),GGG
...
(else_try),
	...
	(eq, ":return_code", 100), #failbankchange
  ...
  ("on_agent_spawned", # server and clients: set agent slots and attributes after spawning
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (try_begin),
      ...
      ###GGG:no global chat
      (try_begin),
        (player_slot_eq, ":player_id", slot_player_no_global_chat, 1),
        (player_set_is_muted, ":player_id", 1, 1), 
      (else_try),
        (this_or_next|player_slot_eq, ":player_id", slot_player_no_global_chat, 0),
                     (player_slot_eq, ":player_id", slot_player_no_global_chat, 2),
        (player_set_is_muted, ":player_id", 0, 0),  
      (try_end),
      ###GGG
    (try_end),
    ]),
   ...
   (try_begin), #schauen ob player exploited
    ...
    (try_end),
	    ...
	    (try_begin),
			...
			(try_end),
			(send_message_to_url, "@http://localhost/backendDsDvC/save.php?playeruid={reg1}&head={reg2}&body={reg3}&foot={reg4}&gloves={reg5}&item1={reg6}&item2={reg7}&item3={reg8}&item4={reg9}&health={reg10}&carriedgold={reg12}&troop={reg20}&faction={reg11}&posx={reg13}&posy={reg14}&posz={reg15}&food={reg16}&horse={reg17}"),
			(try_begin),
				(gt, ":last_instance", 0),
				(call_script, "script_cf_save_item_bank", ":player_id"),
			(try_end),
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (try_begin),
		  ...
		  (server_add_message_to_log, "@{s1} took {reg3} gold out the chest of faction: {s2}"),
        (try_end),
      ...
      (agent_get_slot, ":food_amount", ":agent_id", slot_agent_food_amount),
	   ...
	   (try_begin),
		   ...
		   (try_begin), #prevents maths error
			   (gt, ":food_amount", 0.25),
			   (val_sub, ":food_amount", 0.25), #zieht das Fressen weg
			  (else_try),
			   (le, ":food_amount", 0.25),
