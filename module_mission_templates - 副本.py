...
player_joined = (ti_server_player_joined, 0, 0, [], # server: handle connecting players
   [(store_trigger_param_1, ":player_id"),
    ...
    #GGG:mercenary
    (player_set_slot, ":player_id", slot_player_buy_bot, 0),
    ])
...
agent_killed = (ti_on_agent_killed_or_wounded, 0, 0, [], # server and clients: handle messages, score, loot, and more after agents die
   [(store_trigger_param_1, ":dead_agent_id"),
    ...
    #GGG:mercenary
      ...
      (agent_get_slot, ":player_id", ":dead_agent_id", slot_agent_belong_player),
      (player_set_slot, ":player_id", slot_player_buy_bot, 0),
    (else_try),
    #
      ...
      (try_begin),
	      (eq, ":disconnected_bool", 0), #player didnt disconnect means just kill him
  		  (try_begin),
   ...
   [(multiplayer_is_server),
    ...
    (try_for_range, ":player_id", "$g_loop_player_id", ":loop_end"), # continue from the last player id checked
      (player_is_active, ":player_id"),
        ###GGG:god of eye system
     # (try_begin),
     #   (player_slot_eq, ":player_id", slot_player_god_of_eye, 1),
     #   (player_get_agent_id, ":agent_id", ":player_id"),
     #   (agent_is_active, ":agent_id"),
     #   (agent_set_speed_modifier, ":agent_id", 50),
     #   (agent_set_use_speed_modifier, ":agent_id", 0),
     # (try_end),
      ###
...
agent_check_new_loop = (0, 0, 0.8, [], # server: repeatedly check all agents for attacking with a weapon they can't use - should be kept as simple as possible
   [(multiplayer_is_server),
    (try_for_agents, ":agent_id"),
        ###GGG:foreign invasion
        (try_begin),
          ...
          (agent_get_troop_id, ":troop_id", ":agent_id"),
          (eq, ":troop_id", "trp_temp_troop"), 
          (agent_set_is_alarmed, ":agent_id", 1),
          #GGG:mercenary
          (try_begin),
            (agent_get_slot, ":lord_player_id", ":agent_id", slot_agent_belong_player),
            (gt, ":lord_player_id", 0),
            (player_get_slot, ":lord_faction_id", ":lord_player_id", slot_player_faction_id),
            (player_get_agent_id, ":lord_agent_id", ":lord_player_id"),
            (agent_add_relation_with_agent, ":agent_id",":player_agent_id", 1),

            (agent_get_position, pos15, ":lord_agent_id"),
            (team_give_order, team_default, ":lord_player_id", mordr_hold),
            (team_set_order_position, team_default, ":lord_player_id", pos15),
            (try_for_agents, ":loop_agent"),
              (agent_get_player_id, ":player_id", ":loop_agent"),
              (player_is_active, ":player_id"),
              (agent_is_alive, ":loop_agent"),
              
              (player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
              (eq, ":lord_faction_id", "faction_id"), 
              (agent_add_relation_with_agent, ":agent_id", ":loop_agent", 1),
            (try_end),
          (try_end),
          #
          (try_for_agents, ":cur_agent"),
            ...
            (eq, ":troop_id", "trp_temp_troop"), 
            (agent_add_relation_with_agent, ":cur_agent", ":agent_id", 1),
          (try_end),
        (try_end),
        ### 
    (try_end),
...
###GGG
    ...
    #        (scene_prop_get_instance, ":safe_core", "spr_torture_tool_a", 0),
    #    (prop_instance_get_position, pos2, ":safe_core"),
      
    #  (get_distance_between_positions_in_meters, ":safe_zone", pos1, pos2),
    #  (try_begin),
     #       (le, ":safe_zone", 10),
     #       (agent_set_accuracy_modifier, ":agent_id", 1000),
     # #(else_try),
      # (gt, ":safe_zone", 10),
        #    (agent_set_damage_modifier, "agent_id", 100),
     # (try_end),  
     #(try_end),
     #     ]),    
###GGG
