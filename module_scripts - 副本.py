...
#	]),
 ...
 [(store_script_param, ":player_id", 1),
	...
	(player_get_slot, ":item1", ":player_id", slot_player_bank_1),
	(player_get_slot, ":item2", ":player_id", slot_player_bank_2),
	(player_get_slot, ":item3", ":player_id", slot_player_bank_3),
	(player_get_slot, ":item4", ":player_id", slot_player_bank_4),
	(player_get_slot, ":item5", ":player_id", slot_player_bank_5),
	(player_get_slot, ":item6", ":player_id", slot_player_bank_6),
	(player_get_slot, ":item7", ":player_id", slot_player_bank_7),
	(player_get_slot, ":item8", ":player_id", slot_player_bank_8),
	(player_get_slot, ":item9", ":player_id", slot_player_bank_9),
	(player_get_slot, ":item10", ":player_id", slot_player_bank_10),
	...
	(send_message_to_url, "@http://localhost/backendDsDvC/saveitembank.php?guid={reg1}&i1={reg2}&i2={reg3}&i3={reg4}&i4={reg5}&i5={reg6}&i6={reg7}&i7={reg8}&i8={reg9}&i9={reg10}&i10={reg11}"),
 ]),
   ...
   [(store_script_param, ":player_id", 1), # must be valid
    (try_begin),
      ...
      (assign, reg1, ":player_id"),
			...
			(eq, ":spawnedonce", 1), #Player ist schon einmal gespawnt = keine ruestung holen
			
			(else_try), #player ist noch nicht gespawnt = ruestung spawnen
		    (player_get_unique_id, reg2, ":player_id"),
...
##############################################
# (c) custom PW Scripts by Illuminati        #
# You are not allowed to sell these scripts! #
#---------no one will sell dude--------------#
##############################################
...
###GGG:# check if the agent needs to lose hunger and thirst
   ...
   [(store_script_param, ":integer_count", 1),
	...
	#Hier jetzt alle Return Codes ueberpruefen
      ...
      (else_try),
      	 ...
      	 (assign, ":player_id", reg1),
      	 (str_store_string, s0, "@Kicked cause wrong name"),
      ...
      (else_try),
         (eq, ":return_code", -5),
         (assign, ":player_id", reg1),
         (str_store_string, s0, "@The key has been registered, if has any question, please contract admin, now logout register server."),
         (multiplayer_send_string_to_player, ":player_id", server_event_admin_chat_announce, s0),
         (store_mission_timer_a, ":time"),
         (val_add, ":time", name_server_kick_delay_interval),
         (player_set_slot, ":player_id", slot_player_kick_at_time, ":time"),      
      ...
      (else_try),
         (eq, ":return_code", -7),
         (assign, ":player_id", reg1),
         (str_store_string, s0, "@It's seem you enter a wrong name, please contact admin."),
         (multiplayer_send_string_to_player, ":player_id", server_event_admin_chat_announce, s0),
         (store_mission_timer_a, ":time"),
         (val_add, ":time", name_server_kick_delay_interval),
         (player_set_slot, ":player_id", slot_player_kick_at_time, ":time"),
      ...
      (else_try),
         (eq, ":return_code", -8),
         (assign, ":player_id", reg1),
         (str_store_string, s0, "@The key has been registered too many names."),
         (multiplayer_send_string_to_player, ":player_id", server_event_admin_chat_announce, s0),
         (store_mission_timer_a, ":time"),
         (val_add, ":time", name_server_kick_delay_interval),
         (player_set_slot, ":player_id", slot_player_kick_at_time, ":time"),     
      ...
      (else_try),
      	 ...
      	 (assign, ":player_id", reg1),
      	 (str_store_string, s0, "@You are banned on this server please check our forums or contact an admin!"),
         (multiplayer_send_string_to_player, ":player_id", server_event_admin_chat_announce, s0),
         (store_mission_timer_a, ":time"), # kick the player after a short delay to try ensure they see the rejection message
         (val_add, ":time", name_server_kick_delay_interval),
         (player_set_slot, ":player_id", slot_player_kick_at_time, ":time"),
      ...
      #   (eq, ":return_code", -7),
      #   (assign, ":player_id", reg1),
      #   (str_store_string, s0, "@Welcome to our Server. Your GUID is: {reg2} ."),
      #   (str_store_string, s1, "@You need to get the verification from http://rn.x-pw.cn ."),
      #   (multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s0),
      #   (multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s1),
      #   (player_set_slot, ":player_id", slot_player_god_of_eye, 1),
      #####
      ...
      (else_try),
        	(eq, ":return_code", 89), # bank geladen von datenbank
        	(assign, ":player_id", reg1),
        	(assign, ":item1", reg2),
        	(assign, ":item2", reg3),
        	(assign, ":item3", reg4),
        	...
        	(try_begin),
          	(gt, ":item1", 0),
          	(player_set_slot, ":player_id", slot_player_bank_1, ":item1"),
        	(else_try),
        	  (player_set_slot, ":player_id", slot_player_bank_1, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item2", 0),
          	(player_set_slot, ":player_id", slot_player_bank_2, ":item2"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_2, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item3", 0),
          	(player_set_slot, ":player_id", slot_player_bank_3, ":item3"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_3, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item4", 0),
          	(player_set_slot, ":player_id", slot_player_bank_4, ":item4"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_4, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item5", 0),
          	(player_set_slot, ":player_id", slot_player_bank_5, ":item5"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_5, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item6", 0),
          	(player_set_slot, ":player_id", slot_player_bank_6, ":item6"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_6, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item7", 0),
          	(player_set_slot, ":player_id", slot_player_bank_7, ":item7"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_7, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item8", 0),
          	(player_set_slot, ":player_id", slot_player_bank_8, ":item8"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_8, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item9", 0),
          	(player_set_slot, ":player_id", slot_player_bank_9, ":item9"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_9, ":noitem"),
        	(try_end),
        	
        	(try_begin),
          	(gt, ":item10", 0),
          	(player_set_slot, ":player_id", slot_player_bank_10, ":item10"),
          (else_try),
          	(player_set_slot, ":player_id", slot_player_bank_10, ":noitem"),
        	(try_end),
	
      (else_try),
	        (eq, ":return_code", 90), # Item Bank gespeichert
  ...
  ###GGG:igcommand
      ...
      (else_try),
          ...
          (try_begin),
            (eq, ":realname", 1),
            (player_set_slot, ":player_id", slot_player_god_of_eye, ":realname"),
            (str_store_string, s1, "@unactive system is on"),
            (multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s1),   
          (else_try),
            (neq, ":realname", 1),
            (str_store_string, s1, "@you have no access to use this, please contact the admin"),
            (multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s1), 
          (try_end), 
      ...
      #(else_try),
      	#(eq, ":return_code", 101), #bank enable
      	#(assign, "$g_bank_enabled", 1),
      	#(server_add_message_to_log, "@Bank got enabled by the SO!"),
	
      #(else_try),
      	#(eq, ":return_code", 102), #bank disable
      	#(assign, "$g_bank_enabled", 0),
      	#(server_add_message_to_log, "@Bank got disabled by the SO!"),
	###
      ...
      (else_try), # requesting a spawn point or switching spectator status
        ...
        (else_try),
          ...
          (else_try), # otherwise, toggle spectator status
            ...
            ###GGG:unactive system
            (try_begin),
              (player_slot_eq, ":sender_player_id", slot_player_god_of_eye, 1),
              (player_get_slot, ":faction_id", ":sender_player_id", slot_player_faction_id),
  ...
  ("initialize_scene_globals", # reset global values to initial values when starting a mission
   [
    ...
    (try_for_range, ":faction_id", factions_begin, factions_end),
	  ...
	  # am anfang alle fraktionen peace
   ...
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (else_try), # if the player is currently alive under spawn protection, check if the time is up
      ...
      (try_begin),
		...
		#Fix of starter weapons... new patch:
		(player_get_slot, ":spawnedonce", ":player_id", slot_player_spawnedonce),
		(try_begin),
			(eq, ":spawnedonce", 0),
		(else_try),
  ...
  ("setup_player_joined", # server: when a player connects, update necessary data on clients, restore data from previous play sessions in this mission, and set up other attributes
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (player_set_slot, ":player_id", slot_player_god_of_eye, 0),  ###GGG:unactive system
  ...
  ("cf_save_player_exit", # server: when a player disconnects, save attributes to the inactive players list
   [(store_script_param, ":player_id", 1), # must be valid
   (player_set_slot, ":player_id", slot_player_has_disconnected_ok, 1),  
	 ...
	 (try_begin),
      ...
      (assign, reg18, ":water_amount"),
			...
			(else_try),
  			...
  			(try_end),
    			(send_message_to_url, "@http://localhost/backendDsDvC/save.php?playeruid={reg1}&head={reg2}&body={reg3}&foot={reg4}&gloves={reg5}&item1={reg6}&item2={reg7}&item3={reg8}&item4={reg9}&health={reg10}&carriedgold={reg12}&troop={reg20}&faction={reg11}&posx={reg13}&posy={reg14}&posz={reg15}&food={reg16}&horse={reg17}&water={reg18}"),
    			(try_begin),
    				(gt, ":last_instance", 0),
    				(call_script, "script_cf_save_item_bank", ":player_id"),
    			(try_end),
  ...
  ("cf_castle_is_active",
   [(store_script_param, ":castle_no", 1),
    ...
    (try_for_range, ":part_player_id", 1, ":max_players"), # remove the status of any previous lord of the faction
			...
			(assign, ":rewardforcapture", 500),
			(else_try),
			(ge, ":max_players", 50), # >= 50 spieler?
			(assign, ":rewardforcapture", 1200),
			(else_try),
			(ge, ":max_players", 100), # >= 100 spieler?
			(assign, ":rewardforcapture", 1500),
			(else_try),
			(ge, ":max_players", 150), # >= 150 spieler?
			(assign, ":rewardforcapture", 2500),
			(else_try),
			(ge, ":max_players", 180), # >= 180 spieler?
			(assign, ":rewardforcapture", 3250),
			(try_end),
...
scripts.extend([
  ...
  ("cf_use_inventory", # server: reply with inventory contents of a scene prop when requested by a player
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    ###GGG:house item
    #(try_begin),
    #    (scene_prop_get_slot, ":inventory_slot_end", ":instance_id", 199),
    #    (gt, ":inventory_slot_end", 0),
    #    (val_add, ":inventory_slot_end", 200),
    #    (scene_prop_get_slot, ":item_id", ":instance_id", 200),
    #    (prop_instance_get_variation_id_2, ":var_2", ":instance_id"),
    #    (assign, reg3, ":var_2"),
    #    (assign, reg2, 200),
    #    (assign, reg1, ":item_id"),
    #    (send_message_to_url, "@http://localhost/backendDsDvC/housesaveitem.php?var={reg3}&slot={reg2}&item={reg1}"),
    #    (try_for_range, ":inventory_slot", 200, ":inventory_slot_end"),
    #        (store_add, ":next_inventory_slot", ":inventory_slot", 1),
    #        (try_begin),
    #            (neg|ge, ":next_inventory_slot", ":inventory_slot_end"),
    #            (scene_prop_get_slot, ":next_item_id", ":instance_id", ":next_inventory_slot"),
    #        (else_try),
    #            (assign, ":next_item_id", -9999),
    #        (try_end),
    #        (assign, reg1, ":next_item_id"),
    #        (assign, reg2, ":next_inventory_slot"),
    #        (prop_instance_get_variation_id_2, ":var_2", ":instance_id"),
    #        (assign, reg3, ":var_2"),
    #        (gt, ":var_2", 0),
    #        (send_message_to_url, "@http://localhost/backendDsDvC/housesaveitem.php?var={reg3}&slot={reg2}&item={reg1}"),
    ...
    #    (prop_instance_get_variation_id_2, ":var_2", ":instance_id"),
    #    (gt, ":var_2", 0),
    #    (str_store_string, s0, "@Chest content saved"),
    #    (multiplayer_send_string_to_player, ":player_id", 12, 0),
    #(try_end),
    ###GGG
    ]),
  ...
  ("transfer_inventory", # server: handle inventory transfers within and between scene props and their agent
   [(store_script_param, ":player_id", 1), # must be valid
    ...
    (try_begin), # failures in this section will force disconnection from the scene prop, to stop receiving updates
      ...
      (try_begin), # for failures after this point, stay connected with the scene prop, to receive updates
        ...
        (else_try), # check that the item fits inside when storing in the inventory
		  (try_begin),
		  	(scene_prop_get_slot, ":valbank", ":instance_id", slot_scene_prop_inventory_count),
			(eq, ":valbank", 10),
    ...
    (else_try),
	  ...
	  (call_script, "script_cf_update_item_bank", ":player_id", ":instance_id", 1),
    (try_end),
    ]),
  ...
  (store_script_param, ":status_get", 3),
		(player_get_slot, ":last_instance_id", ":player_id", slot_player_last_instance_id),
	(try_begin),
	(scene_prop_get_slot, ":valbank", ":last_instance_id", slot_scene_prop_inventory_count),
	(eq, ":valbank", 10),
		(scene_prop_get_slot, ":item1", ":last_instance_id", slot_scene_prop_inventory_begin + 0),	
		(scene_prop_get_slot, ":item2", ":last_instance_id", slot_scene_prop_inventory_begin + 1),
		(scene_prop_get_slot, ":item3", ":last_instance_id", slot_scene_prop_inventory_begin + 2),
		(scene_prop_get_slot, ":item4", ":last_instance_id", slot_scene_prop_inventory_begin + 3),
		(scene_prop_get_slot, ":item5", ":last_instance_id", slot_scene_prop_inventory_begin + 4),
		(scene_prop_get_slot, ":item6", ":last_instance_id", slot_scene_prop_inventory_begin + 5),
		(scene_prop_get_slot, ":item7", ":last_instance_id", slot_scene_prop_inventory_begin + 6),
		(scene_prop_get_slot, ":item8", ":last_instance_id", slot_scene_prop_inventory_begin + 7),
		(scene_prop_get_slot, ":item9", ":last_instance_id", slot_scene_prop_inventory_begin + 8),
		(scene_prop_get_slot, ":item10", ":last_instance_id", slot_scene_prop_inventory_begin + 9),
		...
		(try_begin),
		(eq, ":status_get", 1),
		(call_script, "script_cf_remove_bank_instance", ":player_id", ":instance_id"), #removes items out of item bank
		(try_end),
	(try_end),
  ]),
  ...
  [(store_script_param, ":player_id", 1),
  #(store_script_param, ":instance_id", 2), GGG
		(player_get_slot, ":last_instance_id", ":player_id", slot_player_last_instance_id),
	(try_begin),
			(scene_prop_get_slot, ":valbank", ":last_instance_id", slot_scene_prop_inventory_count),
			(eq, ":valbank", 10),
			(assign, reg1, 0),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 0, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 1, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 2, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 3, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 4, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 5, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 6, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 7, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 8, reg1),
			(scene_prop_set_slot, ":last_instance_id", slot_scene_prop_inventory_begin + 9, reg1),
			(assign, "$g_bank_used", 0),
	(try_end),
  ]),
  ...
  ("cf_use_cart", # server: handle players trying to attach or access carts
   [(store_script_param, ":agent_id", 1), # must be valid
	...
	(eq, ":valbank", 10),
		(try_begin),
	#(eq, "$g_bank_enabled", 1),  ##########GGG:item bank key
	# bank ist an d.h weitermachen
	(try_begin),
		(assign, reg15, "$g_bank_used"),
		(eq, reg15, 1),
		# bank benutzt
		(agent_get_player_id, ":player_id", ":agent_id"),
		(player_is_active, ":player_id"),
		(str_store_string, s1, "@The bank is used at the moment. Please try it again in some seconds!"),
		(multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s1),
		(else_try),
		# ist eine bankchest jetzt nur zulassen das man benutzen darf
		(agent_get_player_id, ":player_id", ":agent_id"),
		(player_is_active, ":player_id"),
		(agent_is_alive, ":agent_id"),
		(try_begin),
		  (eq, ":action", 1),
		  (str_store_string, s0, "@You have accessed your item bank, you can store 10 items at all"),
		  ...
		  (multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s0),
		   ...
		   #noch schnell die chest auf player anpassen
			 (try_begin),
				 (gt, ":item1", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 0, ":item1"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item2", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 1, ":item2"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item3", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 2, ":item3"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item4", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 3, ":item4"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item5", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 4, ":item5"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item6", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 5, ":item6"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item7", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 6, ":item7"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item8", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 7, ":item8"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item9", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 8, ":item9"),
			 (try_end),
			 
			 (try_begin),
				 (gt, ":item10", 0),
				 (scene_prop_set_slot, ":instance_id", slot_scene_prop_inventory_begin + 9, ":item10"),
			 (try_end),
		  
		  (assign, "$g_bank_used", 1),
	...
	#bank nicht an
			(agent_get_player_id, ":player_id", ":agent_id"),
		(player_is_active, ":player_id"),
	    (str_store_string, s8, "@The bank is disabled!"),
		(multiplayer_send_string_to_player, ":player_id", server_event_local_chat, s8),
	(try_end),
	
	
	
	(else_try), #Normaler Vorgang ist also keine Bankchest
   ...
   [(store_script_param, ":dead_agent_id", 1),
    ...
    (try_begin),
      ...
      ###GGG:foreign invasion
        ...
        (assign, ":entry_point", 9),
        (assign, ":troop_id", "trp_temp_troop"),
 ...
 ("cf_admin_action", # server: check player requests to use admin tools, applying if successful
   [(store_script_param, ":admin_action", 1), # constants starting with admin_action_
    ...
    (else_try),
      ...
      (else_try),
        (player_set_slot, ":player_id", slot_player_no_global_chat, 2),
      (try_end),      
      ###
    (else_try),
